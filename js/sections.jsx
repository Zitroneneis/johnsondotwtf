// Reusable section blocks for both inline (combined) and dedicated rendering.

// Pick a random iris photo on page load — sticks for the lifetime of the page.
const IRIS_PHOTO = `iris/iris${1 + Math.floor(Math.random() * 6)}.jpg`;
window.IRIS_PHOTO = IRIS_PHOTO;

function PMSection({ asPage }) {
...
window.PMSection = PMSection;

function TheatreSection({ asPage }) {
  const d = window.THEATRE_DATA;
  return (
    <>
...
window.TheatreSection = TheatreSection;

// ---- IRIS CAMEO (combined page only) -------------------------
function IrisCameo() {
  return (
